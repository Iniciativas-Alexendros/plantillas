...
class TestCheckArchivosProhibidos:
    ...
    def test_glob_extension_detectado(self, tmp_path):
        ...
        assert any(r.nivel == Nivel.ERROR and "clave.pem" in r.mensaje for r in resultados)
...
class TestCheckTamanioMaximo:
    def test_archivo_excede_limite(self, tmp_path):
        ...
        assert any(r.nivel == Nivel.ERROR and "grande.bin" in r.mensaje for r in resultados)
...
class TestCheckMergeConflicts:
    def test_marcador_detectado(self, tmp_path):
        (tmp_path / "conflicto.txt").write_text("<<<<<<< HEAD\nfoo\n=======\nbar\n>>>>>>> rama\n")
        v = FakeValidator(tmp_path)
        resultados = check_merge_conflicts(v)
        assert any(r.nivel == Nivel.ERROR and "conflicto.txt" in r.mensaje for r in resultados)
...
class TestCheckSecrets:
    def test_token_detectado(self, tmp_path):
        ...
        assert any(r.nivel == Nivel.ERROR and "config.txt" in r.mensaje for r in resultados)
...
class TestCheckGitignoreMinimo:
    def test_falta_gitignore(self, tmp_path):
        ...
        assert any(r.nivel == Nivel.ERROR and ".gitignore" in r.mensaje for r in resultados)

    def test_entrada_faltante_es_warning(self, tmp_path):
        (tmp_path / ".gitignore").write_text(".env\n")
        v = FakeValidator(tmp_path)
        resultados = check_gitignore_minimo(v, [".env", "*.key"])
        assert any(r.nivel == Nivel.WARNING and "*.key" in r.mensaje for r in resultados)
